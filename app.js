...
// Geschwister-Knoten der gleichen Ebene stehen mit gleichem Abstand nebeneinander
// in einer Reihe; Eltern werden horizontal über ihren Kindern zentriert.
const LAYOUT = {
  nodeWidth: 220,
  nodeHeight: 90,
  colGap: 40,    // horizontaler Abstand zwischen Geschwister-Knoten
  rowGap: 140,   // vertikaler Abstand zwischen Hierarchie-Ebenen
  startX: 0,
  startY: 0
};
...
// Vertikaler Baum: Y = Ebene (Tiefe) * Zeilenabstand -> alle Knoten der gleichen
//     Hierarchieebene stehen exakt auf der gleichen Höhe (gleiches Y).
// X = horizontale Position. Jeder Knoten "besitzt" eine Breite, die der Summe der
//     Breiten all seiner Nachkommen entspricht (oder seiner eigenen Breite, falls
//     er keine Kinder hat). Geschwister werden nebeneinander mit gleichem Abstand
//     platziert; ein Elternknoten wird horizontal über der Mitte seiner Kinder zentriert.
function computeLayout() {
  // Schritt 1: für jeden Knoten die Breite seines Teilbaums berechnen
  ...
  tree.forEach(root => subtreeWidth(root));

  // Schritt 2: Positionen zuweisen. leftEdge = linke Kante des verfügbaren Bereichs
  // für diesen Knoten (inkl. seines gesamten Teilbaums).
  function place(node, depth, leftEdge) {
    const y = LAYOUT.startY + depth * LAYOUT.rowGap;
    ...
    // Kinder nacheinander von leftEdge aus nebeneinander platzieren
    let cursorX = leftEdge;
    node.children.forEach(child => {
      place(child, depth + 1, cursorX);
    ...
    try {
      if (!node.miro || !node.miro.shapeId) {
        ...
        const shape = await miro.board.createShape({
          ...
          content: `<p><b>${escapeHtml(node.code)}</b></p><p>${escapeHtml(node.title)}</p><p>R:</p>`
        });
        ...
        if (shape) {
          shape.content = `<p><b>${escapeHtml(node.code)}</b></p><p>${escapeHtml(node.title)}</p><p>R:</p>`;
          shape.x = x;
          shape.y = y;
      ...
      try {
        const connector = await miro.board.createConnector({
          ...
          start: {
            ...
            snapTo: 'bottom' // verlässt den Elternknoten immer unten-mittig
          },
          end: {
