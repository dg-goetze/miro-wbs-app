...
async function syncToBoard() {
  ...
  function buildCardTitle(node, extraPadLines = 0) {
    const pad = '<p>&nbsp;</p>';
    const extraPadding = pad.repeat(extraPadLines);
    if (!node.code) {
      return `<p>${escapeHtml(node.title)}</p>${pad}<p>R:</p>${pad}${extraPadding}`;
    }
    return `<p><b>${escapeHtml(node.code)}</b></p><p>${escapeHtml(node.title)}</p>${pad}<p>R:</p>${pad}${extraPadding}`;
  }
