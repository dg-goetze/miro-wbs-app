...
async function syncToBoard() {
  ...
  for (const [parentId, children] of childrenByParent) {
    ...
    if (allSameX) {
      ...
      const layoutKey = `vertical,${parent.miro.x},${parent.miro.y},${validChildren.map(c => c.miro.y).join(',')}`;
      ...
      try {
        for (const child of validChildren) {
          const conn = await miro.board.createConnector({
            ...
            end: { item: child.miro.shapeId, position: { x: 0.5, y: 0.0 } },
            style: { strokeColor: '#9aa3b2', strokeWidth: 1, startStrokeCap: 'none', endStrokeCap: 'stealth' }
          });
    ...
    const childTopY = validChildren[0].miro.y; // alle Kinder auf gleicher Y-Ebene
    ...
    const layoutKey = `${parentCenterX},${railY},${currentChildXs.join(',')}`;
    const previousKey = parent.miro._railLayoutKey || '';
    ...
    try {
      ...
      // d) Horizontale Schiene: Connectors zwischen allen sortierten Punkten auf der Schiene
      //    (Stem + Branches, sortiert nach X)
      const allRailPoints = [
        { id: stem.id, x: parentCenterX },
        ...branchPoints.map(bp => ({ id: bp.id, x: bp.x }))
      ].sort((a, b) => a.x - b.x);

      // Duplikate entfernen (falls Stem und ein Branch auf exakt gleicher X-Position liegen)
      const uniqueRailPoints = [allRailPoints[0]];
      for (let i = 1; i < allRailPoints.length; i++) {
        if (Math.abs(allRailPoints[i].x - allRailPoints[i - 1].x) > 0.5) {
          uniqueRailPoints.push(allRailPoints[i]);
        }
      }

      for (let i = 0; i < uniqueRailPoints.length - 1; i++) {
        const railConn = await miro.board.createConnector({
          shape: 'straight',
          start: { item: uniqueRailPoints[i].id, snapTo: 'right' },
          end: { item: uniqueRailPoints[i + 1].id, snapTo: 'left' },
          style: { strokeColor: '#9aa3b2', strokeWidth: 1, startStrokeCap: 'none', endStrokeCap: 'none' }
        });
