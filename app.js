...
async function syncToBoard() {
  ...
  function buildCardTitle(node, extraPadLines = 0, rContent = 'R:') {
    const pad = '<p>&nbsp;</p>';
    const extraPadding = extraPadLines > 0 ? pad.repeat(extraPadLines) : '';
    if (!node.code) {
      return `<p>${escapeHtml(node.title)}</p>${pad}<p>${rContent}</p>${extraPadding}`;
    }
    return `<p><b>${escapeHtml(node.code)}</b></p><p>${escapeHtml(node.title)}</p>${pad}<p>${rContent}</p>${extraPadding}`;
  }

  // Extrahiert den R:-Inhalt aus einem bestehenden Kartentitel (z.B. "R: David Götze")
  function extractRContent(cardTitle) {
    if (!cardTitle) return 'R:';
    // Sucht nach dem letzten <p>R:...</p> Block im Titel
    const match = cardTitle.match(/<p>(R:[^<]*)<\/p>/i);
      ...
      if (!node.miro || !node.miro.shapeId) {
        ...
        if (titleChanged || positionChanged || colorChanged) {
          try {
            ...
            const preservedR = oldCard ? extractRContent(oldCard.title) : 'R:';
            ...
            const newCard = await miro.board.createCard({
              ...
              y: newCenterY,
              width: LAYOUT.nodeWidth,
              title: buildCardTitle(node, 0, preservedR),
              description: preservedDescription,
              style: { cardTheme: color, fillBackground: false }
            });
            await newCard.setMetadata('wbsId', node.id);
            node.miro.shapeId = newCard.id;
            console.log(`[WBS Sync] Karte ${node.code} neu erstellt (R: "${preservedR}")`);
          } catch (err) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const newCard = await miro.board.createCard({
          ...
          width: LAYOUT.nodeWidth,
          title: buildCardTitle(node, extraLines, preservedR),
          description: preservedDescription,
          style: { cardTheme: color, fillBackground: false }
        });
        await newCard.setMetadata('wbsId', node.id);
  ...
  const RAIL_Y_FRACTION = 0.45; // Schiene bei 45% des Abstands zwischen Eltern-Unterkante und Kind-Oberkante
  ...
  for (const [parentId, children] of childrenByParent) {
    ...
    // Tatsächliche Kartenposition und -höhe aus Miro auslesen, nicht schätzen.
    // Die geschätzte nodeHeight stimmt bei gepolsterten Karten oft nicht mit der
    // tatsächlichen Höhe überein, was dazu führt, dass die Sammelschiene zu nahe am
    // Kartenboden liegt und die vertikale Stem-Linie unsichtbar kurz wird.
    let parentVisualBottomY, childVisualTopY, parentCenterX;
    try {
      const parentCard = await miro.board.getById(parent.miro.shapeId);
      parentCenterX = parentCard.x;
      parentVisualBottomY = parentCard.y + (parentCard.height || LAYOUT.nodeHeight) / 2;

      const firstChildCard = await miro.board.getById(validChildren[0].miro.shapeId);
      childVisualTopY = firstChildCard.y - (firstChildCard.height || LAYOUT.nodeHeight) / 2;
    } catch (e) {
      // Fallback auf Schätzwerte, falls Karten nicht gelesen werden können
      parentCenterX = parent.miro.x + LAYOUT.nodeWidth / 2;
      parentVisualBottomY = parent.miro.y + LAYOUT.nodeHeight;
      childVisualTopY = validChildren[0].miro.y;
    }

    const gap = Math.max(40, childVisualTopY - parentVisualBottomY);
    const railY = parentVisualBottomY + Math.max(30, gap * RAIL_Y_FRACTION);

    const currentChildXs = validChildren.map(c => c.miro.x + LAYOUT.nodeWidth / 2).sort((a, b) => a - b);
    const layoutKey = `${parent.miro.shapeId},${Math.round(parentCenterX)},${Math.round(railY)},${validChildren.map(c => `${c.miro.shapeId}:${c.miro.x}`).join(',')}`;
    const previousKey = parent.miro._railLayoutKey || '';
