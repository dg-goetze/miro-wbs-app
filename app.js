...
async function syncToBoard() {
  ...
  async function createCardForNode(node, topLeftX, topLeftY, color) {
    // Erste Erstellung mit geschätzter Center-Position
    const estimatedCenterX = topLeftX + LAYOUT.nodeWidth / 2;
    const estimatedCenterY = topLeftY + LAYOUT.nodeHeight / 2;
    ...
    // Tatsächliche Höhe auslesen und Y korrigieren, damit die Oberkante (nicht die
    // Mitte) am beabsichtigten Y liegt. Ohne diese Korrektur wären Karten mit
    // unterschiedlich langem Text nicht oben bündig ausgerichtet.
    const actualHeight = card.height || LAYOUT.nodeHeight;
    const correctCenterY = topLeftY + actualHeight / 2;
    if (Math.abs(correctCenterY - estimatedCenterY) > 2) {
      card.y = correctCenterY;
      await card.sync();
    }
    // Tatsächliche Höhe speichern für spätere Verwendung (Schienen-Berechnung etc.)
    node._actualHeight = actualHeight;
    return card;
  }
      ...
      if (!node.miro || !node.miro.shapeId) {
        ...
        if (titleChanged || positionChanged || colorChanged) {
          try {
            ...
            // Tatsächliche Höhe auslesen und Y korrigieren (top-aligned)
            const actualHeight = newCard.height || LAYOUT.nodeHeight;
            const correctCenterY = y + actualHeight / 2;
            if (Math.abs(correctCenterY - newCenterY) > 2) {
              newCard.y = correctCenterY;
              await newCard.sync();
            }
            node._actualHeight = actualHeight;
  ...
  for (const [parentId, children] of childrenByParent) {
    ...
    const parentActualHeight = parent._actualHeight || LAYOUT.nodeHeight;
    const parentCenterX = parent.miro.x + LAYOUT.nodeWidth / 2;
    const parentBottomY = parent.miro.y + parentActualHeight;
