...
async function syncToBoard() {
  ...
  for (const { node, depth } of flat) {
    ...
    try {
      if (!node.miro || !node.miro.shapeId) {
        ...
        if (card) {
          const newTitle = buildCardTitle(node);
          const newCenterX = x + LAYOUT.nodeWidth / 2;
          const newCenterY = y + LAYOUT.nodeHeight / 2;

          // Nur tatsächlich ändern und syncen, wenn sich etwas geändert hat. Ein
          // bedingungsloses sync() bei jedem Durchlauf kann dazu führen, dass Miro die
          // Karte neu rendert und dabei manuell aufgeklappte Karten wieder einklappt.
          const titleChanged = card.title !== newTitle;
          const positionChanged = card.x !== newCenterX || card.y !== newCenterY;
          const colorChanged = card.style.cardTheme !== color;
  ...
  // 2. Verbindungen zwischen Eltern und Kindern anlegen, falls noch nicht vorhanden.
  //
  // WICHTIG: Miro-Karten haben keine fest einstellbare Höhe (sie wird automatisch aus
  // dem Textinhalt berechnet) – ein direktes Andocken mit snapTo:'bottom' an die Karte
  // selbst würde bei unterschiedlich langen Titeln zu schräg/uneben liegenden Linien
  // führen. Außerdem routet Miros 'elbowed'-Connector-Typ bei mehreren von einem Punkt
  // ausgehenden Linien oft mit unerwünschtem seitlichen Versatz, statt zuverlässig erst
  // gerade nach unten und dann zu verzweigen.
  //
  // LÖSUNG: Eine sichtbare horizontale "Sammelschiene" (dünnes Shape) pro Elternknoten,
  // auf fester Höhe unterhalb der Karte. Ein gerader Connector verbindet die Karte mit
  // der Mitte dieser Schiene. Für jedes Kind gibt es einen eigenen, unsichtbaren
  // Branch-Punkt GENAU AUF der Schiene (an der X-Position des Kindes), von dem eine
  // gerade, senkrechte Linie zum Kind führt. Da alle Connectors hier 'straight' sind
  // (keine automatische Routing-Entscheidung von Miro), ist die Linienführung
  // vollständig vorhersehbar und entspricht dem klassischen Baumdiagramm-Look.
  const RAIL_OFFSET_BELOW_TOP = 70;     // Abstand der Sammelschiene unterhalb der Karte
  const BRANCH_OFFSET_BELOW_RAIL = 20;  // Abstand der Branch-Punkte unterhalb der Schiene

  // Erzeugt/aktualisiert die horizontale Sammelschiene für einen Elternknoten.
  // Die Schiene erstreckt sich von der X-Position des am weitesten links liegenden
  // Kindes bis zum am weitesten rechts liegenden Kind.
  async function getOrCreateRail(parentNode, children) {
    const parentCenterX = parentNode.miro.x + LAYOUT.nodeWidth / 2;
    const railY = parentNode.miro.y + RAIL_OFFSET_BELOW_TOP;

    const childCenters = children.map(c => c.miro.x + LAYOUT.nodeWidth / 2);
    const leftMost = Math.min(parentCenterX, ...childCenters);
    const rightMost = Math.max(parentCenterX, ...childCenters);
    const railWidth = Math.max(8, rightMost - leftMost);
    const railCenterX = (leftMost + rightMost) / 2;

    if (parentNode.miro.railId) {
      try {
        const existing = await miro.board.getById(parentNode.miro.railId);
        if (existing) {
          if (existing.x !== railCenterX || existing.y !== railY || existing.width !== railWidth) {
            existing.x = railCenterX;
            existing.y = railY;
            existing.width = railWidth;
            await existing.sync();
          }
          return existing;
        }
      } catch (e) { /* Schiene existiert nicht mehr -> neu anlegen */ }
    }

    const rail = await miro.board.createShape({
      shape: 'rectangle',
      x: railCenterX,
      y: railY,
      width: railWidth,
      height: 8, // Mindesthöhe von Miro; dünne, sichtbare Linie
      style: {
        fillColor: '#9aa3b2',
        fillOpacity: 1,
        borderOpacity: 0
    ...
    try { await miro.board.sendToBack(rail); } catch (e) { /* unkritisch */ }
    parentNode.miro.railId = rail.id;
    return rail;
  }

  // Erzeugt/aktualisiert einen unsichtbaren Branch-Punkt auf der Schiene, exakt über
  // dem zugehörigen Kind (gleiche X-Position wie das Kind).
  async function getOrCreateBranchPoint(parentNode, childNode) {
    const targetX = childNode.miro.x + LAYOUT.nodeWidth / 2;
    const targetY = parentNode.miro.y + RAIL_OFFSET_BELOW_TOP + BRANCH_OFFSET_BELOW_RAIL;

    parentNode.miro.branchIds = parentNode.miro.branchIds || {};
    const existingId = parentNode.miro.branchIds[childNode.id];

    if (existingId) {
      try {
        const existing = await miro.board.getById(existingId);
        if (existing) {
      ...
      } catch (e) { /* Branch-Punkt existiert nicht mehr -> neu anlegen */ }
    }
    ...
    try { await miro.board.sendToBack(branch); } catch (e) { /* unkritisch */ }
    parentNode.miro.branchIds[childNode.id] = branch.id;
    return branch;
  }

  // Vorab-Durchlauf: für jeden Elternknoten mit Kindern die Schiene und alle
  // Branch-Punkte aktualisieren/anlegen – unabhängig davon, ob die Verbindungen
  // selbst schon existieren. Sonst blieben Schiene/Branches bei späteren Syncs
  // (z.B. nach Hinzufügen neuer Geschwister/Enkel, die das Layout verschieben)
  // an veralteten Positionen stehen.
  ...
  for (const [parentId, children] of childrenByParent) {
    ...
    const childrenWithMiro = children.filter(c => c.miro);
    if (childrenWithMiro.length === 0) continue;
    try {
      await getOrCreateRail(parent, childrenWithMiro);
      for (const child of childrenWithMiro) {
        await getOrCreateBranchPoint(parent, child);
      }
    } catch (err) {
      console.error(`[WBS Sync] Konnte Schiene/Branches für ${parent.code} nicht aktualisieren:`, err);
    }
  }
  ...
  for (const { node, parentId } of flat) {
    ...
    if (!parent || !parent.miro || !node.miro) {
      console.warn(`[WBS Sync] Verbindung für ${node.code} übersprungen – Eltern- oder Kind-Shape fehlt noch.`);
      continue;
    }

    try {
      // a) Gerade Linie von der Karte des Elternknotens zur Mitte der Sammelschiene
      //    (nur einmal pro Elternknoten nötig, nicht pro Kind)
      if (!parent.miro.railConnectorId && parent.miro.railId) {
        const railConnector = await miro.board.createConnector({
          shape: 'straight',
          start: { item: parent.miro.shapeId, snapTo: 'bottom' },
          end: { item: parent.miro.railId, snapTo: 'top' },
          style: {
            strokeColor: '#9aa3b2',
            strokeWidth: 1,
            startStrokeCap: 'none',
        ...
        try { await miro.board.sendToBack(railConnector); } catch (e) { /* unkritisch */ }
        parent.miro.railConnectorId = railConnector.id;
      }

      // b) Gerade Linie vom Branch-Punkt (auf der Schiene, über dem Kind) zum Kind
      if (!parent.connectorIds[node.id]) {
        const branchId = parent.miro.branchIds && parent.miro.branchIds[node.id];
        if (!branchId) {
          console.warn(`[WBS Sync] Branch-Punkt für ${node.code} fehlt – wird im nächsten Sync nachgeholt.`);
          continue;
        }
        const connector = await miro.board.createConnector({
          shape: 'straight',
          start: { item: branchId, snapTo: 'bottom' },
          end: { item: node.miro.shapeId, snapTo: 'top' },
          style: {
            strokeColor: '#9aa3b2',
            strokeWidth: 1,
            startStrokeCap: 'none',
        ...
        try { await miro.board.sendToBack(connector); } catch (e) { /* unkritisch */ }
        parent.connectorIds[node.id] = connector.id;
        console.log(`[WBS Sync] Verbindung angelegt: ${parent.code} -> ${node.code}`);
      }
    } catch (err) {
      console.error(`[WBS Sync] Verbindungs-Fehler ${parent.code} -> ${node.code}:`, err);
      errors.push(`Verbindung ${parent.code} → ${node.code}: ${err.message}`);
    }
  }
...
async function removeOrphanedShapes(flat) {
  ...
  flat.forEach(({ node }) => {
    ...
    if (node.miro && node.miro.railId) knownIds.add(node.miro.railId);
    if (node.miro && node.miro.railConnectorId) knownIds.add(node.miro.railConnectorId);
    if (node.miro && node.miro.branchIds) {
      Object.values(node.miro.branchIds).forEach(id => knownIds.add(id));
    }
  });
