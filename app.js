...
async function syncToBoard() {
    ...
    try {
      if (!node.miro || !node.miro.shapeId) {
        ...
        if (card) {
          ...
          // Prüfen, ob sich tatsächlich etwas geändert hat. Ein bedingungsloses
          // sync() bei jedem Durchlauf führt dazu, dass Miro die Karte neu rendert
          // und dabei manuell aufgeklappte Karten wieder einklappt.
          //
          // - Titel-Vergleich: gegen den intern gespeicherten Rohtext (nicht gegen
          //   card.title aus Miro, da Miro HTML ggf. normalisiert/umformatiert,
          //   wodurch ein String-Vergleich immer "geändert" liefern würde).
          // - Position: mit Toleranz (±1dp), da Floating-Point-Rundungsdifferenzen
          //   zwischen Miro's gespeichertem Wert und meiner Berechnung sonst
          //   bei jedem Sync fälschlicherweise "Änderung" signalisieren.
          // - Farbe: case-insensitive, da Hex-Farben in Groß/Klein variieren können.
          const titleChanged = (node.miro.lastSyncedTitle !== node.title);
          const positionChanged = Math.abs(card.x - newCenterX) > 1 || Math.abs(card.y - newCenterY) > 1;
          const colorChanged = (card.style.cardTheme || '').toLowerCase() !== (color || '').toLowerCase();
          ...
          if (titleChanged || positionChanged || colorChanged) {
            ...
            await card.sync();
            console.log(`[WBS Sync] Karte ${node.code} aktualisiert (Titel: ${titleChanged}, Pos: ${positionChanged}, Farbe: ${colorChanged})`);
          }
  ...
  //
  // WICHTIG: Bei jedem Sync werden ALLE bestehenden Connectors gelöscht und komplett
  // neu erstellt. Grund: Wenn neue Kinder hinzukommen, verschiebt sich das Layout
  // (Elternknoten wandern), aber Miro routet bestehende elbowed-Connectors anders als
  // frisch erstellte. Die Mischung aus "alten" und "neuen" Routen führt zu
  // unterschiedlichen Höhen der horizontalen Linienabschnitte. Da Connectors keine
  // nutzereigenen Daten enthalten (anders als die Karten mit Beschreibungen), gibt es
  // keinen Datenverlust durch das Neuerstellen.

  // 2a. Alle bestehenden Connectors löschen
  for (const { node, parentId } of flat) {
    if (!parentId) continue;
    const parent = findNode(tree, parentId);
    if (!parent) continue;
    const existingConnectorId = parent.connectorIds[node.id];
    if (existingConnectorId) {
      try {
        const existing = await miro.board.getById(existingConnectorId);
        if (existing) await miro.board.remove(existing);
      } catch (e) { /* Connector existiert evtl. schon nicht mehr */ }
      delete parent.connectorIds[node.id];
    }
  }

  // 2b. Alle Connectors neu erstellen
  for (const { node, parentId } of flat) {
    if (!parentId) continue;
    const parent = findNode(tree, parentId);
