...
/* ---------- Hierarchische Codes berechnen (1, 1.1, 1.1.1, ...) ----------
   Die Wurzelebene startet bei ROOT_START_NUMBER (einstellbar, Standard 1) statt fix bei 0.
   Jede tiefere Ebene hängt sich mit einem Punkt an den Code des Elternknotens an:
   1, 1.1, 1.1.1, ... Bei mehreren Wurzelknoten wird hochgezählt: 1, 2, 3, ...
*/
...
async function syncToBoard() {
  ...
  async function createCardForNode(node, x, y, color) {
  ...
  for (const { node, depth } of flat) {
    ...
    try {
      if (!node.miro || !node.miro.shapeId) {
        ...
        // Bestehende Karte aktualisieren (Text, Position UND Farbe je Ebene)
        const card = await miro.board.getById(node.miro.shapeId);
        if (card) {
          card.title = `${node.code}  ${node.title}`;
          card.x = x;
          card.y = y;
