...
async function syncToBoard() {
  ...
  function buildCardTitle(node) {
    ...
    return `${escapeHtml(node.code)}<br><br>${escapeHtml(node.title)}`;
  }
  ...
  for (const { node, depth } of flat) {
    ...
    try {
      if (!node.miro || !node.miro.shapeId) {
        ...
        // Bestehende Karte aktualisieren (Text, Position, Farbe UND Beschreibung "R:",
        // falls diese z.B. durch eine frühere App-Version noch fehlt)
